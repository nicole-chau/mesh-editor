...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      attrPos(-1), attrNor(-1), attrCol(-1), attrWeight(-1), attrJointID(-1),
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifCamPos(-1),
      unifOverallTransMat(-1), unifBindMat(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    unifOverallTransMat = context->glGetUniformLocation(prog, "u_OverallTransformMats");
    unifBindMat = context->glGetUniformLocation(prog, "u_BindMats");
}
...
void ShaderProgram::setOverallTransMat(const std::vector<glm::mat4> &overallTransMat) {
    useMe();

    if (unifOverallTransMat != -1) {
        context->glUniformMatrix4fv(unifOverallTransMat,
                                    overallTransMat.size(),
                                    GL_FALSE,
                                    &overallTransMat[0][0][0]);
    }
}

void ShaderProgram::setBindMat(const std::vector<glm::mat4> &bindMat) {
    useMe();

    if (unifBindMat != -1) {
        context->glUniformMatrix4fv(unifBindMat,
                                    bindMat.size(),
                                    GL_FALSE,
                                    &bindMat[0][0][0]);
    }
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (attrWeight != -1 && d.bindWeight()) {
        ...
        context->glVertexAttribPointer(attrWeight, 2, GL_FLOAT, false, 0, nullptr);
    }
